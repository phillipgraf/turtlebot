...
class Tb3(Botnode):
    def __init__(self):
        ...
        self.min_dist_front = 0.7
        self.min_dist_back = 0.7
    ...
    def odom_callback(self, msg):
        ...
        if self.rot:
            ...
            if len(self.beam_intensities) > 1 and detect_red_with_lds_front(self):
                ...
                if not cell_center(self, thresh=0.1):
                    drive(self, 30)
                    self.first_time_center = True
                elif self.first_time_center:
                    self.first_time_center = False
                    if self.object_front and self.object_left and self.object_right:
                        stop(self)
                        self.back_go = True
                    elif self.object_front and self.object_left:
                        stop(self)
                        self.rot = True
                        ...
                        self.rot = True
                        self.rotate_direction = 5
                    elif self.object_back and self.object_left:
                        stop(self)
                        self.rot = True
                        self.rotate_direction = -5
                    elif self.object_back and self.object_right:
                        stop(self)
                        self.rot = True
                        self.rotate_direction = 5
                    elif self.object_left and self.object_right:
                        drive(self, 30)
                    elif not self.object_right and random.randint(0,1) == 1:
                        stop(self)
                        self.rot = True
                        self.rotate_direction = -5
                    elif not self.object_left and random.randint(0,1) == 1:
                        stop(self)
                        self.rot = True
                        self.rotate_direction = 5
